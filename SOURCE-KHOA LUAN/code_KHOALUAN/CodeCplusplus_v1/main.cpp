...
void gfsr_init(long seed)
{
    ...
    cout << "Initializing SR-sequences with seed " << seed << endl;
           ...
           double *tgralin, double *sdin, double *chi2ain)
{
    ...
    for (it = ittot; it <= itmx + ittot - 1; it++)
    {
        ...
        for (;;)
        {
            ...
            for (k = 0; k < npg; k++)
            {
                ...
                for (j = 0; j < ndim; j++)
                {
                    ...
                    // printf("\nxn = %lf\n",xn);v
                    // ia[dim] được gán bằng xn ==> ia là chỉ số để chạy trên trục NDMX đoạn
                    ia[j] = ((int)xn < NDMX) ? ((int)xn) : (NDMX);
                    ia[j] = (ia[j] > 1) ? (ia[j]) : (1);
                    if (ia[j] > 1) // chỉ số khac 1 (đang nằm trong [a,b])
                    {
                        xo = xi[j][ia[j] - 1] - xi[j][ia[j] - 2];  // xo = xi[i+1] - xi[i]
                        rc = xi[j][ia[j] - 2] + (xn - ia[j]) * xo; //
                    }
                    else // bắt đâu [a,b]
                    {
                        xo = xi[j][ia[j] - 1]; // => xo = xi[dim][0]
                        rc = (xn - ia[j]) * xo;
                    }
                    //  printf("\nxo = %lf\n",xo);
                    //  printf("\nxn - ia[j] = %lf\n",xn - ia[j]);
                ...
                for (j = 0; j < ndim; j++)
                {
                    di[ia[j] - 1][j] += f; // cộng dồn là do 2 điểm gieo
                    if (mds >= 0)
                    { //printf("\nKhong c\n");
                        d[ia[j] - 1][j] += Ax.f2;
                    } //sum up npg magnitude f2 whithin each bin of a hypercube
                }
            } /* end of loop within hypercube */
            // printf("\nGieo diem thu %d\n", k);
            // Ax.f2b = (Ax.fb - Ax.f2b)/(npg-1);
            Ax.f2b = sqrt(Ax.f2b * npg);
        ...
        *chi2ain = chi2a;
        ...
        for (j = 0; j < ndim; j++)
        {
            const char *filePath2 = "C:/Users/Ha Vy/Desktop/ri.txt";
            FILE *file2;
            file2 = fopen(filePath2, "w+");
            rc = 0.0;
            for (i = 0; i < nd; i++)
...
int main()
{
    ...
    vegas(reg, DIMENSION, testfun, 0, 1000, 10, NPRN_INPUT | NPRN_RESULT, estim, std_dev, chi2a);
    printf("Result: %g +/- %g\n", estim[0], std_dev[0]);
    //randomNumberTest();
    printf("\n============================\n");
    vegas(reg, DIMENSION, testfun, 0, 10000, 50, NPRN_INPUT | NPRN_RESULT, estim, std_dev, chi2a);
    printf("Result: %g +/- %g\n", estim[0], std_dev[0]);
